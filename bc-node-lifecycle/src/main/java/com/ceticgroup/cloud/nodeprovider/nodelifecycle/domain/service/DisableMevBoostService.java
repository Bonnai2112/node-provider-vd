...
public final class DisableMevBoostService implements DisableMevBoostUseCase {
    ...
    public void disable(NodeId id, OwnerId requester) {
        ...
        if (ref == null) {
            throw new IllegalNodeTransitionException(
                    "Cannot disable MEV-Boost: deployment is missing");
        }
    ...
    private void applyAsync(NodeId id, DeploymentRef ref, NodeSpec newSpec) {
        try {
            ...
            try {
                transition(id, n -> n.fail("disable mev-boost failed: " + safeMessage(e)));
