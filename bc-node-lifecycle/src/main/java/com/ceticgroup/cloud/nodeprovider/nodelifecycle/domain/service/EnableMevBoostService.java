...
public final class EnableMevBoostService implements EnableMevBoostUseCase {
    ...
    @Override
    public void enable(NodeId id, OwnerId requester, String mevMinBid, int mevBuildFactor) {
        ...
        if (ref == null) {
            throw new IllegalNodeTransitionException(
                    "Cannot enable MEV-Boost: deployment is missing");
        }
        node.enableMevBoost(mevMinBid, mevBuildFactor);
        NodeSpec newSpec =
                new NodeSpec(
    ...
    private void applyAsync(NodeId id, DeploymentRef ref, NodeSpec newSpec) {
        try {
            ...
            try {
                transition(id, n -> n.fail("enable mev-boost failed: " + safeMessage(e)));
